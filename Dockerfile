...
FROM python:3.10.10-slim-buster@sha256:2425b3b12adf669291256767a3c07257ed4f3f53d26970fd44dc528b824e28dd
ARG TARGETARCH
...
RUN --mount=type=cache,target=/var/cache/apt \
    apt-get update && \
        ...
        apt-get install -y --no-install-recommends \
            ...
            # FIXME Add GCC and python-dev to be able to build psutil on ARM64. \
            #       This causes ~67MB additional size (gzip compressed tar of the docker image). \
            #       This could be avoided by moving this towards a builder stage which builds the wheels.
            # FIXME Node 18 actually shouldn't be necessary in Community, but we assume its presence in lots of tests
            gcc python-dev nodejs \
            # FIXME "ps" is not installed by default, but used by some tests (and probably by some users / localstack itself?)
            procps
...
RUN --mount=type=cache,target=/root/.cache pip3 install --upgrade awscli awscli-local requests
...
# TODO install the latest version of the persistence plugin here

# TODO add LPM calls, which are decided to be necessary (maybe dynamodb?)
